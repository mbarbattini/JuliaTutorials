...
# special bidiagonal type
A = Bidiagonal(diagonal, upperDiagonal, :U)
# typeof(A)
A = Symmetric(A, :U)
# convert to a matrix so eigvals() can be called
A = convert(Matrix, A)
eigvals(A)
eigvecs(A)
...
# sort along the second dimension from smallest to largest
sort!(A, dims=2)

# no special matrix multiplication symbol or function
